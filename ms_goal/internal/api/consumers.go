...
import (
	...
	"log/slog"
	"messaging"
	c "ms_goal/internal/core/messaging/consumers"
...
func NewConsumer(
	...
	transactionHandler := c.NewTransactionEventHandler(
		services.goalTransaction,
		logger,
	)

	handlersMap := map[string]messaging.EventHandler{
		events.TransactionGoalCreatedTopic: transactionHandler,
		events.TransactionGoalDeletedTopic: transactionHandler,
	}

	return &consumers{
		kafkaConsumer: messaging.NewConsumer(
			consumerGroup,
			handlersMap,
			logger,
		),
